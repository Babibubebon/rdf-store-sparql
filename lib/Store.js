...
class Store {
  constructor (endpointUrl, { updateUrl, factory = rdf, maxQueryLength = Infinity, fetchOptions } = {}) {
    ...
    this.client = new SparqlHttpClient({
      ...
      fetch: (query, options) => {
        options = {...this.fetchOptions, ...options}
        options.headers = {...this.fetchOptions.headers, ...options.headers}
        return fetch(this.factory, query, options)
      }
    })
  }
